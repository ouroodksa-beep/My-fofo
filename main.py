...
def expand_url(url):
    try:
        r = requests.get(url, allow_redirects=True, timeout=5)  # سرعة أعلى
        return r.url
    except:
        return url
...
def handler(msg):
    ...
    for short_url in urls:
        ...
        # ⚡ تحسين الأداء باستخدام ThreadPoolExecutor
        with ThreadPoolExecutor(max_workers=2) as executor:
            future = executor.submit(get_product, asin)
            try:
                product = future.result(timeout=10)  # أقصى وقت 10 ثواني
            except:
                product = None
...
print("🔥 بوت احترافي شغال + CTA ذكي + سرعة عالية")
