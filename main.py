...
def generate_post_with_ai(title, price, original_title):
    """استخدام OpenAI GPT لكتابة بوست قصير"""
    try:
        headers = {
        ...
        prompt = f"""اكتب منشور تسويقي قصير جداً (3-4 أسطر بالكثير) باللهجة السعودية:

المنتج: {original_title}
السعر: {price} ريال

المطلوب:
1. سطر افتتاحي جذاب (هلا بالزين، تخيل، يا عزيزي)
2. ترجمة مختصرة جداً للمنتج (3-4 كلمات بالكثير)
3. جملة تسويقية واحدة قصيرة (سعر قديم + سبب الشراء)
4. السعر بالريال

مثال:
هلا بالزين كله! 🤩
حذاء نايك رياضي وصل! 👟
🔥 كان بـ 280 ريال الحين بـ 199 ريال بس!
💰 199 ريال
...
اكتب المنشور مباشرة:"""
        ...
        data = {
            "model": "gpt-4o-mini",
            "messages": [
                {"role": "system", "content": "أنت مسوق سعودي، تكتب منشورات قصيرة جداً ومختصرة باللهجة السعودية. تستخدم إيموجي كثيرة. لا تطيل الكلام."},
...
def generate_post(product):
    ...
    ai_text = generate_post_with_ai("", price, original_title)
    
    if not ai_text:
        ai_text = f"✨ منتج رائع بـ {price} ريال! 🔥"
    
    # إضافة الرابط في سطر منفصل
    full_post = f"{ai_text}\n\n🔗 {url}"
...
if __name__ == "__main__":
    ...
    print("🤖 Bot running! Short posts with AI.")
