...
def generate_post(product_data, original_url):
    """Generate thousands of completely unique, highly varied Khaleeji marketing posts using Groq AI."""
    ...
    angles = [
        "زاوية الصدمة والذهول من السعر الرخيص",
        "زاوية النصيحة السرية بين الأصدقاء (تراها لقطة لا تفوتك)",
        "زاوية الاحتياج الملزم (ليش كل بيت محتاجها ضروري؟)",
        "زاوية الإلحاح الشديد وقرب نفاذ الكمية",
        "زاوية المدح والثناء المبالغ فيه على جودة المنتج",
        "زاوية المقارنة الذكية ووفرة التوفير مقارنة بالسوق",
        "زاوية البساطة والعفوية التامة في سرد التجربة",
        "زاوية الحماس والتشويق (يا جماعة شوفوا وش لقيت لكم)"
    ]
    
    dialects_shades = [
        "بلهجة خليجية بيضاء وعفوية",
        "بلهجة سعودية حماسية وشعبية",
        "بلهجة تجمع بين المزح والترغيب السريع",
        "بلهجة توحي بأن العرض سري وموجه للخاص فقط"
    ]
    ...
    prompt = f"""
أنت عبقري التسويق الإلكتروني وصانع محتوى إبداعي لا ينضب. اكتب بوست تسويقي احترافي لمنتج من أمازون السعودية، مع ضرورة أن يكون **مختلفاً كلياً وفريداً من نوعه** في طريقة صياغته عن أي نص تقليدي.

الزاوية التسويقية المطلوبة: ({chosen_angle})
الطابع اللفظي المتبع: ({chosen_dialect})
...
قواعد صارمة جداً:
1. **تجنب القوالب الثابتة نهائياً:** ابتكر طريقة سرد جديدة تماماً، ابدأ البوست بجملة غير متوقعة تجذب الانتباه فوراً وبدون رغي طويل.
2. **الاسم الصريح:** اذكر اسم العنصر الأساسي بوضوح ودون حشو كلمات غير مفيدة.
3. **تنسيق الأسعار:** أبرز السعر الحالي بشكل جذاب وواضح باستخدام الخط العريض (Bold)، وأبرز الخصم إن وجد.
4. **الطول:** من 2 إلى 3 أسطر فقط مركزة وقوية.
5. لا تضع رابط الشراء في ردك نهائياً.
6. أعطني النص مباشرة بدون مقدمات أو علامات تنصيص.
"""
    ...
    try:
        completion = groq_client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "أنت خبير تسويق ابتكاري تتحدث بآلاف الأساليب المتجددة دوماً."},
        ...
        ai_generated_text = f"يا لبيييه على هاللّقطة في {name}! 🔥\n\nمتوفر بسعر خرافي: **{clean_current}**.\n\nالحق العرض قبل يفوتك!"
