...
def expand_short_url(url):
    ...
    try:
        # Headers محسنة عشان نخدع amzn.to
        headers = {
        ...
        # استخدام HEAD request الأول عشان نجيب الـ Location header
        if "amzn.to" in url or "amzn.com" in url:
            # محاولة 1: HEAD request
            try:
            ...
            # محاولة 2: GET request مع allow_redirects
            try:
...
def handler(msg):
    ...
    for url in urls:
        ...
        # لو الرابط الموسع نفسه، نحاول نبني رابط أمازون مباشر
        if expanded_url == url and ("amzn.to" in url or "amzn.com" in url):
            # نفترض إن الرابط المختصر بيحتوي على ASIN في النهاية
            # أو نستخدم رابط افتراضي
            bot.edit_message_text("❌ فشل في توسيع الرابط المختصر. جرب رابط أمازون مباشر.", msg.chat.id, wait.message_id)
            continue
        
        # لو مفيش amazon في الرابط الموسع
        ...
        # نستخدم الرابط الأصلي في البوست
