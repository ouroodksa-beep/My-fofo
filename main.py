...
# --- الإعدادات ---
# تأكدي أن التوقن بين العلامات " " ويحتوي على النقطتين :
...
def run_flask():
    # هذا الجزء يحل مشكلة الـ Port scan في Render المجاني
    port = int(os.environ.get('PORT', 8080))
    app.run(host='0.0.0.0', port=port)

def get_amazon_data(short_url):
    try:
        # فك الرابط المختصر ضروري لسحب البيانات
...
def handle_links(m):
    if "amazon" in m.text or "amzn.to" in m.text:
        ...
        if caption and img:
            ...
            bot.reply_to(m, "❌ المعذرة، الرابط ما سحب البيانات بالعربي، جربي رابط ثاني يا ذيبة.")

if __name__ == "__main__":
    # مسح أي تعارض قديم عند التشغيل
    try:
        bot.delete_webhook(drop_pending_updates=True)
    except:
        pass
